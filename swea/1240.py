...
def decode_password(line):
    # 56비트의 암호 코드 부분을 추출하여 숫자로 변환
    digits = []
...
def process_case():
    ...
    for line in matrix:
        if "1" in line:
            # 1이 포함된 라인에서 마지막 1을 기준으로 56비트 추출
            index = line.rfind("1")
            code_line = line[index - 55:index + 1]
            digits = decode_password(code_line)
