...
def count_stuck_magnets(table):
    ...
    for col in range(size):
        ...
        for row in range(size):
            if table[row][col] == 1:  # N극 자성체를 발견
                moving_N = True
            elif table[row][col] == 2:  # S극 자성체를 발견
                if moving_N:  # 이전에 N극이 있었다면 교착 상태 발생
                    stuck_count += 1
                moving_N = False  # N극을 초기화
...
for tc in range(1, 11):
    ...
    table = [list(map(int, input().strip().split())) for _ in range(size)]
    
    result = count_stuck_magnets(table)
    print(f"#{tc} {result}")